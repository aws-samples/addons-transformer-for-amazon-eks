...
export default class ChartValidatorService extends BaseService {
  ...
  private async findCapabilities(): Promise<ServiceResponse<string>> {
    ...
    this.supportedKubernetesVersions.map(k8sVersion => {
      ...
      const capsWithCrds = spawnSync('grep', ['-ilen', '".Capabilities"', "<<<", withCrds.stdout], {shell: true, encoding: "utf-8"});
      const capsWithoutCrds = spawnSync('grep', ['-ilen', '".Capabilities"', "<<<", withoutCrds.stdout], {shell: true, encoding: "utf-8"});
